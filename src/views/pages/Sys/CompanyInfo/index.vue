        ...
        .form-contetn(v-loading='azdLoading' style="width:100%; padding-left: 50px")
          el-form(ref='azdform' :model='azdform' label-width='180px')
            el-form-item(label='安装点名称设置: ' prop="azdnameSet")
              el-checkbox-group(v-model="anzdset" @change="handleCheckedCitiesChange")
...
export default {
  ...
  data() {
    ...
    return {
      ...
      azd: {},
      azdOption: [
        'ishsowjzw_azd', 'ishsowlc_azd', 'isshowazd_azd', 'ishsowjzw_sblb', 'ishsowlc_sblb', 'isshowazd_sblb'
      ]
    }
  },
  computed: {
  ...
  methods: {
    ...
    handleCheckedCitiesChange(e) {
      console.log(e)
      e.forEach(n => {
        this.$see(this.azd, n, this.azdTitle[n])
      })
    },
    setAzd(data) {
    ...
    getAzdData() {
      ...
      getAzdConfig().then(res => {
        ...
        this.setAzd(data)
        this.anzdset = anzdset
        console.log(this.anzdset)
      }).catch(err => {
        this.azdLoading = false
        console.error(err)
      })
